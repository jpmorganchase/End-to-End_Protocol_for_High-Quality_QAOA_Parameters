...
def get_data(N, seed=1, real=False):
    ...
    if real is False:
        ...
        stock_symbols = [
        "AAPL", "GOOGL", "AMZN", "MSFT", "TSLA",'NFLX', "NVDA", "JPM", "V", "JNJ",
        "WMT", "PG", "MA", "UNH", "HD", "DIS", "BRK-B", "VZ", "KO", "MRK", "INTC",
        "CMCSA", "PEP", "PFE", "CSCO", "XOM", "BA", "MCD", "ABBV", "IBM", "GE", "MMM"
        ]
...
def get_real_problem(N, K, q, pre=False):
    po_problem = {}
    po_problem["N"] = N
    po_problem["K"] = K
    po_problem["q"] = q
    po_problem["real"] = True
    po_problem["means"], po_problem["cov"] = get_data(N, real=True)
    po_problem["pre"] = pre
    scale = 1
    if pre == "constant":
...
def get_problem_GS_K(po_problem):
    ...
    while 1:
        ...
        for eig_vec in eigen_vector:
            nonzero_idx.append(
                [i for i, v in enumerate(np.abs(eig_vec) ** 2) if v > 1e-8]
            )
    ...
    if len(eigen_vector) == 1:
        ...
        nonzero_idx = [
            i for i, v in enumerate(np.abs(eigen_vector[0]) ** 2) if v > 1e-8
        ]
...
def get_trottered_ring_xy_mixer(N, T, beta=1, ring=True):
    ...
    for _ in range(T):
        for term in terms_even:
            exp_term = (
                expm(-beta * 1j * term.to_matrix_op()._primitive._data) @ exp_term
            )
    ...
    for _ in range(T):
        for chain in chains:
            ...
            for _ in range(T):
                for term in terms_even:
                    exp_term = (
                        expm(-beta * 1j * term.to_matrix_op()._primitive._data)
                        @ exp_term
                    )
                for term in terms_odd:
                    exp_term = (
...
def precompute_energies_parallel(obj_f, nbits, num_processes, chunksize=None):
    ...
    with Pool(num_processes) as pool:
        ens = np.array(
            list(
                tqdm(pool.imap(obj_wrapped, range(2**nbits), chunksize=chunksize), total=2**nbits)
            )
        )
    return ens
...
def precompute_energies_parallel_constrained(obj_f, nbits, K, num_processes, postfix=[]):
    ...
    energies : np.array
        ...
        where amplitudes are absolute values squared of qiskit statevector
    """
    bit_strings = (np.hstack([x, postfix]) for x in yield_all_bitstrings_cosntrained(nbits - len(postfix),K) )
    with Pool(num_processes) as pool:
