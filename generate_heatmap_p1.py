    ...
    for seed in range(10):
        ...
        if Path(po_path).exists() and Path(energy_path).exists():
            ...
            scale = 1 / (
                np.sqrt(
                    np.mean(
                        ((po_problem_unscaled["q"] * po_problem_unscaled["cov"]) ** 2).flatten()
                    )
                )
        ...
        def f(gamma, beta):
            ...
            print(
                f"gamma-{gamma:.2f}, beta-{beta:.2f}, time-{time.time()-start_time:.2f}",
                flush=True,
            )
            return [sv.get_norm_squared().dot(precomputed_energies), overlap.real]

        npoints = 50
        min_gamma, max_gamma = -1.4, -0.8
        min_beta, max_beta = 0.9, 1.3
        ...
        hm = np.array(results[:, 0]).reshape(npoints, npoints).T
        overlap_hm = np.array(results[:, 1]).reshape(npoints, npoints).T

        np.save(
            f"data/heatmap/heatmap_{N}_{K}_{q}_p1_seed{seed}_rand_rule.npy",
            (hm, overlap_hm),
            allow_pickle=False,
        )
